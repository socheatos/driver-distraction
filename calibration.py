...
class Video(Webcam):
    ...
    def find_corners(self):
        ...
        if self.ret == True:
            ...
            # print(self.threeD_pts[-1])
            self.corners2 = cv2.cornerSubPix(self.gray,
                                self.corners,
                                CHECKERBOARD,
                                (-1,-1), STOP_CRITERIA)
    ...
    def find_params(self):
        ...
        ret, self.intrinsic, self.distortion, self.r_vecs, self.t_vecs = cv2.calibrateCamera(
            self.threeD_pts, self.twoD_pts, self.gray.shape[::-1], None, None) 
        ...
        savetxt('data/calibration_data/rotation_vectors.csv', mean_r, delimiter=',')
    ...
    def get_camParameters(self,path):
        ...
        self.distortion = genfromtxt(os.path.join(path,'camera_distortion.csv'),delimiter=',')
    ...
    def param_explode(self):
        ...
        self.sx, self.sy = self.focalLength/self.fx, self.focalLength/self.fy
